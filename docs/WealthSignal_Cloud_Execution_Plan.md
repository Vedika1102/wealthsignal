...
The bounded environment-client-4 gate passed in Free Edition run `818005580828490` at commit `c73b98b`. It processed exactly 20,000 rows ending at 2023 Q4, trained on 16,037 rows, evaluated and reloaded 3,963 rows, observed both logistic classes, and completed MLflow, preprocessing, Ridge, logistic, and Delta write/reload checks in 53.307 pipeline seconds. Four later full-volume attempts failed before the first fold checkpoint: one graph schema conflict and three Spark Connect model-response overflows in combined preprocessing, the imputer, and the scaler. Remediations are committed through `77d86c0`; evidence is preserved in `docs/ai-governance/cloud4-free-edition-nine-fold-gate.json`. Do not rerun all nine folds until a full-volume, first-fold-only gate passes.
...
3. **Prove one full-volume fold before resuming Cloud 4.** Add and run an environment-client-4 first-fold-only gate using the latest remediations. It must exercise graph preparation, preprocessing, required models, diagnostics, metrics, and one atomic reloadable checkpoint. Only then may the restartable nine-fold run resume.
...
```text
Read docs/WealthSignal_AWS_Databricks_Architecture_Plan.md, docs/WealthSignal_Cloud_Execution_Plan.md, docs/CODEX_PROJECT_NAVIGATION.md, the Protocol V2 governance files, cloud4_contract.py, its tests, and both retained Cloud 4 gate reports completely. Work only on architecture milestones A0 and the local implementation portion of A1.

Reconcile all stale status text with the four full-volume failures. Then implement an environment-client-4 full-volume, first-fold-only gate using the latest remediations. It must exercise graph preparation, preprocessing, required model fits, diagnostics, metrics, and one atomic reloadable checkpoint. Add focused tests and a submission file, but do not submit jobs, resume nine folds, create AWS resources, access prospective truth, or incur cost without explicit approval. Escalate compute only from measured first-fold evidence.
```
