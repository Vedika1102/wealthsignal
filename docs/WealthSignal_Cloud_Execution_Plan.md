...
1. **Prepare the Free Edition compatibility gate.** Change the serverless job from environment client `2` to `4`. Prove on a bounded historical slice that MLflow, `VectorAssembler`, preprocessing, one Ridge fit, one logistic fit, and a small write/reload succeed. This smoke result is engineering evidence only and may not select a model or change the frozen protocol.
2. **Make Cloud 4 restartable and quota-aware.** Combine all logistic thresholds for one prediction frame into one aggregation, persist fold-level results incrementally, separate graph construction, baseline fitting, and reconciliation into dependent restartable tasks, and avoid recomputing successful folds after a late failure. Investigate the global single-partition node-numbering window before the official run.
3. **Run the official Cloud 4 job on Free Edition.** Evaluate persistence, EMA, popularity, Ridge, gradient boosting, and logistic action diagnostics on identical expanding validation folds, with MLflow lineage. Preserve every measured failure and stop on quota exhaustion rather than weakening the protocol.
...
5. **Escalate compute only from evidence.** If the optimized environment-4 run repeatedly exhausts Free Edition quota, cannot complete a fold for capacity reasons, or encounters a required unsupported serverless API, propose a bounded CPU-only classic job-compute configuration, cost ceiling, auto-termination policy, and cleanup plan and wait for explicit approval.
...
8. **Keep AWS optional and bounded.** If added later, use a cost-capped sandbox for one isolated deployment or MLOps capability; do not make it the default data or GPU platform.
...
```text
Read docs/WealthSignal_Cloud_Execution_Plan.md, docs/CODEX_PROJECT_NAVIGATION.md, the Protocol V2 governance files, cloud3_gold.py, cloud3_gate.py, cloud4_contract.py, their tests, the accepted Cloud 3 reports, and the retained Cloud 4 run evidence completely. Work only on the Cloud 4 Free Edition compatibility and efficiency gate.

Keep the default platform as Databricks Free Edition. Update the serverless submission from environment client 2 to environment client 4, then add a bounded smoke gate proving MLflow, Spark ML preprocessing, one Ridge fit, one logistic fit, and write/reload compatibility without selecting a model. Reduce repeated actions by aggregating logistic thresholds together, checkpoint fold-level evidence, and separate restartable graph, baseline, and reconciliation tasks where practical. Run the complete nine-fold Cloud 4 comparison only after that gate passes. Escalate to paid classic job compute only if measured environment-4 evidence proves quota, capacity, or unsupported-API blockage, and request explicit approval with a cost ceiling before creating anything billable. Do not access prospective Q2 2026 truth, start RunPod, create AWS resources, or introduce a graph-learning dependency. Save measured evidence and stop at the Cloud 5 authorization boundary.
```
